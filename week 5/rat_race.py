...
class MazeApp(tkinter.Frame):
    ...
    def display_score(self, score_frame, score_var, label_text):
        ...
        score_lbl = tkinter.Label(score_frame, textvariable=score_var, font=FONT)
    ...
    def redraw(self):
        ...
        for r in range(len(self.the_maze.maze)):
            for c in range(len(self.the_maze.maze[r])):
                self.the_maze_vars[r][c].set(self.the_maze.get_character(r, c))
    ...
    def rat_1_keystroke(self, event):
        ...
        self.the_maze.move(
            self.the_maze.rat_1, RAT_1_KEYS[event.char][0], RAT_1_KEYS[event.char][1]
        )
    ...
    def rat_2_keystroke(self, event):
        ...
        self.the_maze.move(
            self.the_maze.rat_2, RAT_2_KEYS[event.char][0], RAT_2_KEYS[event.char][1]
        )
